...
import {Program} from "../../component-funcionality/models/programs/program.model";
...
export class ProgramPrincipalComponent implements OnInit {
    ...
    navigateToForm() {
      this._router.navigate(['program-form']).then(() => {
    ...
    reactiveDataFuncionary(program: Program) {
      this._operationalunitService.reactiveLogicalDataFuncionary(program).subscribe((dataFuncionary: any) => {
        this.findAllDataInactiveOperationalUnit();
      });
    }
